...
uint8_t valueLeftIR, valueRightIR;
uint8_t prevCarDirection;

uint8_t maxSpeed             = 255;
uint8_t minSpeed             = 150;
uint8_t leftMotorSpeed       = 150;
uint8_t rightMotorSpeed      = 150;
uint8_t diffSpeed            = 60;
uint8_t Kp                   = 20;
uint8_t Ki                   = 0.4;
uint8_t Kd                   = 0.5;
uint8_t prevErr              = 0;
...
void setup() {
    motor_L.setSpeed(maxSpeed);
    motor_R.setSpeed(maxSpeed);
    motor_L.run(RELEASE);
    motor_R.run(RELEASE);
}

void loop() {
    valueLeftIR = directDigitalRead(14);    // A0 is Pin 14
    valueRightIR = directDigitalRead(19);   // A5 is Pin 19

    currErr = valueLeftIR - valueRightIR;
    valuePID = (Kp * currErr) + (Ki * totalErr) + (Kd * (currErr - prevErr));
    prevErr = currErr;

    leftMotorSpeed = maxSpeed - (2 * valuePID);
    rightMotorSpeed = maxSpeed + (10 * valuePID);
    if (leftMotorSpeed > maxSpeed) leftMotorSpeed = maxSpeed;
    if (minSpeed < leftMotorSpeed) leftMotorSpeed = minSpeed;
    if (rightMotorSpeed > maxSpeed) rightMotorSpeed = maxSpeed;
    if (minSpeed < rightMotorSpeed) rightMotorSpeed = minSpeed;
    ...
    if (valueLeftIR) {
        if (valueRightIR) {
            ...
            motor_L.setSpeed(minSpeed);
            motor_R.setSpeed(rightMotorSpeed - diffSpeed);
            motor_L.run(BACKWARD);
            motor_R.run(FORWARD);
        }
    } else if (valueRightIR) {
        motor_L.setSpeed(leftMotorSpeed - diffSpeed);
        motor_R.setSpeed(minSpeed);
        motor_L.run(FORWARD);

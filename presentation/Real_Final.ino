...
void car()
{
    ...
    val1 = directDigitalRead(A0);    // 라인센서1
    val2 = directDigitalRead(A3);    // 라인센서2
...
void IRstate3()  
{
    ...
    else if(time_current - time_previous <= 1000) // 300ms 전진
    {
        motor_L.run(FORWARD);
        motor_R.run(FORWARD);
        int val1_check = directDigitalRead(A0);    // 라인센서1
        ...
        if(val1_check == 1 || val2_check == 1)  // 1이면 감지
        {
            // Serial.print(", In! Var1 = ");
            // Serial.print(val1_check);
            // Serial.print(", var2 = ");
            // Serial.println(val2_check);
            obstacle_flag = 4;
            time_previous = millis();
        }
    }
    ...
    else if(time_current - time_previous > 1500)  // 다음 상태 진입
    {
        obstacle_flag = 5;
    }        
}

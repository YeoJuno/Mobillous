...
void car() {
    ...
    if ((left_IR_value || right_IR_value) && ((-1) * filter_boundary <= filtered_error && filtered_error <= filter_boundary)) {     // 직진
        motor_L.setSpeed(max_speed);
        motor_L.run(FORWARD);
        motor_R.setSpeed(max_speed);
        motor_R.run(FORWARD);
    } else if (left_IR_value && !right_IR_value) {     // 좌회전
        motor_L.setSpeed(left_speed - diff_speed);
        ...
        motor_L.run(FORWARD);
        motor_R.setSpeed(right_speed - diff_speed);
...
// 동작 on/off 인터럽트 함수
void onoff() {
    if(directDigitalRead(ONOFF) == HIGH && !onoff_flag) {
        onoff_flag = 1;
        distance = 0;
        velocity = 0;
